...
export function getDateDifference(date: Date): string {
  ...
  if (diffInDays > 0) return `Sent ${diffInDays} days ago`;
  else if (diffInHours > 0) return `Sent ${diffInHours} hrs ago`;
  else if (diffInMinutes > 0) return `Sent ${diffInMinutes}m ago`;
